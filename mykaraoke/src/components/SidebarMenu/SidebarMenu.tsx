...
export default function SidebarMenu({ setSideBarMenuMeasure }: ISidebarMenu) {
	...
	return (
		...
		>
			...
			<SidebarMenuItem link="/mock" label={"Mock Interview"} icon={faClipboardQuestion} />
			<SidebarMenuItem link="/profile" label={"Profile"} icon={faUser} />
			<SidebarMenuItem link="/discussion" label={"Discussion"} icon={faComments} />
			<SidebarMenuItem link="/roles" label={"Roles"} icon={faScroll} />
		</div>
	);
}
